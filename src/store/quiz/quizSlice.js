...
export const quizSlice = createSlice({
  ...
  reducers: {
    ...
    checkAnswer: state => {
      ...
      state.questions[state.currentQuestionIndex].state = isCorrect
    ...
    computeResults: state => {
      const correctAnswers = state.questions.filter(
        question => question.state === questionStates.CORRECT
      ).length;
      const incorrectAnswers = state.questions.filter(
        question => question.state === questionStates.INCORRECT
      ).length;
      const result = (correctAnswers / state.totalQuestions) * MAX_SCORE;
      const score =
        result < MINIMUM_SCORE ? MINIMUM_SCORE.toFixed(2) : result.toFixed(2);
