...
const crearRegistro = (cabeza, primerConjunto = [], segundoConjunto = [], esHead = -1, indiceActual = 0) => {
  return {
...
export const Pop = (cabeza, esHead) => {
  ...
  if (cabeza === null) {
    historialPasos.push(crearRegistro(cabeza, [], [], -1, 1));
    return { historialPasos, nuevaCabeza: cabeza };
  }
  
  // Paso 3: Guardar el valor a eliminar
  const valorEliminado = cabeza.getValue();
  historialPasos.push(crearRegistro(cabeza, [valorEliminado], [], 0, 2));
  
  // Paso 4: Actualizar el tope para que apunte al siguiente elemento
  cabeza = cabeza.getNext();
  historialPasos.push(crearRegistro(cabeza, [valorEliminado], [], esHead, 3));
...
export const Sumergir = (cabeza, esHead) => {
  const historialPasos = [];
  ...
  historialPasos.push(crearRegistro(cabeza, [valorTope], [], 0, 2));
  
  // Paso 4: Actualizar el tope para que apunte al segundo elemento
  cabeza = cabeza.getNext();
  historialPasos.push(crearRegistro(cabeza, [valorTope], [], 0, 3));
  
  // Paso 5: Buscar el último nodo de la pila
  let nodoActual = cabeza;
  while (nodoActual.getNext() !== null) {
    nodoActual = nodoActual.getNext();
    historialPasos.push(crearRegistro(cabeza, [valorTope], [nodoActual.getValue()], -1, 4));
  }
  
  // Paso 6: El nodo tope ahora apunta a null y se coloca al final
  nodoTope.setNext(null);
  nodoActual.setNext(nodoTope);
  historialPasos.push(crearRegistro(cabeza, [valorTope], [nodoActual.getValue()], -1, 5));
  
  // Paso 7: Mostrar estado final
  historialPasos.push(crearRegistro(cabeza, [], [], -1, 5));
  
  return { historialPasos, nuevaCabeza: cabeza };
};

// Animación Insertar: Insertar un elemento en una posición específica
// Posición 0 = fondo, posición 1 = segundo desde fondo, etc. Tope = última posición
export const Insertar = (cabeza, valor, posicion, esHead) => {
  const historialPasos = [];
  ...
  if (cabeza === null) {
    if (posicion === 0) {
      cabeza = nuevoNodo;
      historialPasos.push(crearRegistro(cabeza, [], [], 0, 2));
      historialPasos.push(crearRegistro(cabeza, [], [], -1, 5));
      return { historialPasos, nuevaCabeza: cabeza };
    } else {
      historialPasos.push(crearRegistro(cabeza, [], [], -1, 1));
  ...
  if (posicion === 0) {
    ...
    let nodoActual = cabeza;
    historialPasos.push(crearRegistro(cabeza, [valor], [], esHead, 3));
    
    while (nodoActual.getNext() !== null) {
      nodoActual = nodoActual.getNext();
      historialPasos.push(crearRegistro(cabeza, [valor], [nodoActual.getValue()], -1, 3));
    }
    
    // Insertar al final
    nodoActual.setNext(nuevoNodo);
    historialPasos.push(crearRegistro(cabeza, [], [nuevoNodo.getValue()], -1, 4));
  } 
  // Paso 7: Insertar en posición intermedia
    ...
    historialPasos.push(crearRegistro(cabeza, [valor], [nodoActual.getValue()], -1, 4));
...
// Posición 0 = fondo, posición 1 = segundo desde fondo, etc. Tope = última posición
export const Extraer = (cabeza, posicion, esHead) => {
  const historialPasos = [];
  ...
  // Paso 6: Eliminar en otras posiciones
    ...
    historialPasos.push(crearRegistro(cabeza, [], [], esHead, 2));
    ...
    if (posicion === 0) {
      ...
      if (tamanoPila === 1) {
        ...
        valorEliminado = cabeza.getValue();
        historialPasos.push(crearRegistro(cabeza, [valorEliminado], [], 0, 3));
        cabeza = null;
        historialPasos.push(crearRegistro(cabeza, [valorEliminado], [], -1, 4));
      } else {
        // Encontrar el penúltimo nodo
        let nodoActual = cabeza;
        while (nodoActual.getNext().getNext() !== null) {
          nodoActual = nodoActual.getNext();
          historialPasos.push(crearRegistro(cabeza, [], [nodoActual.getValue()], -1, 2));
        }
        
        // Eliminar el último nodo
        valorEliminado = nodoActual.getNext().getValue();
        historialPasos.push(crearRegistro(cabeza, [valorEliminado], [nodoActual.getValue()], -1, 3));
