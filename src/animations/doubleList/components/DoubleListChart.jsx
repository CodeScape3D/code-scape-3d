...
export const DoubleListChart = () => {
  const { head, firstSet, secondSet, thirdSet, highlightedNode, pointer } =
    useSelector(state => state.doubleList);
          ...
          {nodes.map((node, index) => (
            ...
            >
              ...
              <NodeComponent
                ...
                isHead={index === 0}
                hasPrev={node.getPrev() !== null}
              />

              {/* Flecha conectora doble */}
              {node.getNext() && (
                <DoubleArrowComponent arrowWidth={nodeStyles.arrowWidth} />
              )}
            </div>
          ))}
        </div>
    ...
    <div className="relative flex-shrink-0">
      ...
      {isHead && (
        <div className="absolute -top-8 sm:-top-10 left-1/2 transform -translate-x-1/2 z-10">
          <div
            className="bg-purple-600 text-white px-1 sm:px-2 py-0.5 rounded-full font-bold shadow-lg whitespace-nowrap"
      ...
      >
        ...
        {/* Efecto de resaltado */}
        {isHighlighted && (
          <div className="absolute inset-0 rounded-lg sm:rounded-xl bg-purple-300 opacity-30 animate-ping"></div>
        )}
      </div>
    </div>
  );
...
// Componente de flecha doble (bidireccional)
const DoubleArrowComponent = ({ arrowWidth }) => {
  return (
    <div
    ...
    >
      ...
      <div className="flex items-center">
        ...
        ></div>
        <div className="w-0 h-0 border-l-4 border-t-2 border-b-2 border-l-purple-500 border-t-transparent border-b-transparent"></div>
      </div>
      ...
      <div className="flex items-center">
        <div className="w-0 h-0 border-r-4 border-t-2 border-b-2 border-r-purple-500 border-t-transparent border-b-transparent"></div>
        <div
          className="h-0.5 bg-purple-500"
          style={{ width: `${arrowWidth}px` }}
        ></div>
